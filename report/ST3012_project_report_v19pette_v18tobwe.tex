...
We discovered that the e-mail address field is not verified and can contain wrong information. Furthermore, it is not checked if the person already filled a form. This could be solved via a limitation to one response (see figure \ref{fig:response-limit}). The problem with it is that  subjects need a Google account to fill the form if this option is selected. In reality, the suggested pop-up on the website should have a token which becomes invalid after a certain period or when the form was filled. The email verification could be accomplished via a simple e-mail containing a verification link which might be mandatory to take click on before filling the answers.
...
Thank you for cooperating by filling out this questionnaire. Please read carefully this guideline.

The aim of this research is improve the "personal search" sub-section of Dalarna University's internet site (https://www.du.se/). Your answers will be used and analyzed by the researchers of this project.

Your answers will be stored in a database and will be available for the researchers. Your personal details will be stored, however will be saved in a separate database. Only people who are taking part in this project will have access to personal identification which will not be disclosed to anyone else. The rest of the data will be compiled and aggregated, so from the report it will not be possible to recognize individual answers or people. The database will be deleted after 5 years.

Think about about the questions as carefully as possible and look for the anwser which mostly describe your opinion. For every question there is 5 possible answers, 1 to 5. Check the one which matches with your opinion. In the last question you can add any other notes about the "personal search" functionality. Thank you.
